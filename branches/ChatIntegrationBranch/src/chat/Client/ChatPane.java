...
public class ChatPane extends JTextPane {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5442061043246935267L;
	String icoSad;
	String icoSmily;
	String icoTongue;
	String icoWinking;
	...
	public void sendText(String un, String message, boolean whisper) {
		...
		if (un == null || chatClient.username == null) {
			return;
		}
		...
		if (chatClient.username.equals(un)) {
		...
		if (message.startsWith("afk") || message.startsWith("brb")) {
			chatClient.afks.add(un);
			chatClient.updateList();
		} else {
			if (chatClient.afks.contains(un)) {
				chatClient.afks.remove(un);
				chatClient.updateList();
			}
		}
	}
